...
require File.expand_path(File.join(File.dirname(__FILE__), "convex_hull_tests"))

#
# Reabre el modulo de Test para agregar los metodos que hacen las pruebas
#
module Test
  ...
  def self.run
    ...
    assert("convex_hul.h: Calcula el cierre " + 
           "convexo en una lista vacia",TestConvexHull.empty_set)
    assert("convex_hull.h: Calcula el cierre" + 
           " convexo de 3 puntos", TestConvexHull.three_points)
    assert("convex_hull.h: Calcula cierre convexo de un conjunto" +
           " aleatorio de puntos", TestConvexHull.random_set_points)
